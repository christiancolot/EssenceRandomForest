...
cdef class Splitter:
    ...
    cdef int init(self,
                   ...
                   DOUBLE_t* sample_weight, DOUBLE_t* variable_weight,
        ...
        cdef DOUBLE_t* variable_weight_c = safe_realloc(&self.variable_weight_c, n_features)

        for i in range(n_features):
            features[i] = i
#        printf("init n_features %d\n",n_features) 

        for k in range(n_features):
            variable_weight_c[k] = variable_weight[k]
#            printf("init variable_weight_c %Le\n",variable_weight_c[k])    
...
cdef class BaseDenseSplitter(Splitter):
    ...
    cdef int init(self,
        ...
        # Call parent init
        Splitter.init(self, X, y, sample_weight, variable_weight)
...
cdef class BestSplitter(BaseDenseSplitter):
    ...
    cdef int node_split(self, double impurity, SplitRecord* split,
        ...
        cdef DOUBLE_t* variable_weight_c = self.variable_weight_c    
        ...
        #cdef DOUBLE_t* variable_weight = self.variable_weight
        ...
        cdef DOUBLE_t rand_for_var = 0
        cdef DOUBLE_t cum_proba = 0
        cdef DOUBLE_t proba = 0
        cdef DOUBLE_t sum_proba = 0
...
#            printf("variablr_weight_c %Le\n", variable_weight_c) 
#            printf("n_drawn_constants %d\n", n_drawn_constants)
#            printf("n_found_constants %d\n", n_found_constants)

            # Find sum of proba to later divide
            sum_proba = 0
            for k in range(n_drawn_constants,f_i - n_found_constants):
#                    printf("proba %Le\n", proba)
                sum_proba += variable_weight_c[k]
#            printf("sum_proba %Le\n", sum_proba) 
            ...
            # Draw a feature at weighted random
            rand_for_var = rand_uniform(0, 1, random_state)      
            
            f_j = n_drawn_constants
#            printf("%f\n","f_j_first", f_j)
            cum_proba= 0
            for k in range(n_drawn_constants,f_i - n_found_constants):
                cum_proba += variable_weight_c[k] / sum_proba
                if cum_proba > rand_for_var:
                    break
                f_j += 1
#            else:
#                f_j = f_i - n_found_constants
#            printf("%d\n", f_j)
#            if f_j > 130:
#                f_j = 130
#            printf("%d\n", f_j)
...
#                           random_state)
#            printf("f_j qui marche %d\n", f_j)
            if f_j < n_known_constants:
                # f_j in the interval [n_drawn_constants, n_known_constants[
                tmp = features[f_j]
                proba = variable_weight_c[f_j]
                features[f_j] = features[n_drawn_constants]
                features[n_drawn_constants] = tmp

                variable_weight_c[f_j] = variable_weight_c[n_drawn_constants]
                variable_weight_c[n_drawn_constants] = proba
                ...
                if Xf[end - 1] <= Xf[start] + FEATURE_THRESHOLD:
                    ...
                    variable_weight_c[f_j] = variable_weight_c[n_total_constants]
                    variable_weight_c[n_total_constants] = proba
                    ...
                    variable_weight_c[f_i], variable_weight_c[f_j] = variable_weight_c[f_j], variable_weight_c[f_i]
        ...
        #memcpy(variable_weight_c, constant_features, sizeof(DOUBLE_t) * n_known_constants)
